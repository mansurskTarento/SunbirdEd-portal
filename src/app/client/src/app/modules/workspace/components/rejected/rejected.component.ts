...
export class RejectedComponent extends WorkSpace implements OnInit, AfterViewInit {
  ...
  */
  contentIds: string;
  /**
   * Contains list of rejected conten(s) of logged-in user
  */
  rejectedContent: Array<IContents> = [];
  /**
     * To show / hide loader
  */
  showLoader = true;
  ...
  */
  ...
  /**
    * This method sets the make an api call to get all Rejected content with page No and offset
    */
  fetchRejectedContent(limit: number, pageNumber: number, bothParams?: object) {
    ...
    this.search(searchParams).subscribe(
      (data: ServerResponse) => {
        ...
        if (allContent.length > 0) {
          this.rejectedContent = allContent;
          ...
          this.rejectedContent = this.workSpaceService.getDataForCard(allContent, constantData, dynamicFields, metaData);
  ...
  */
  public deleteContent(contentIds) {
        ...
        this.delete(contentIds).subscribe(
          (data: ServerResponse) => {
            this.showLoader = false;
            this.rejectedContent = this.removeContent(this.rejectedContent, contentIds);
            if (this.rejectedContent.length === 0) {
              this.fetchRejectedContent(this.config.appConfig.WORKSPACE.PAGE_LIMIT, this.pageNumber);
            }
  ...
  */
     deleteQuestionSetContent(questionSetId) {
      ...
      this.deleteQuestionSet(questionSetId).subscribe(
        (data: ServerResponse) => {
          this.showLoader = false;
          this.rejectedContent = this.removeContent(this.rejectedContent, questionSetId);
          if (this.rejectedContent.length === 0) {
            this.ngOnInit();
          }
          this.toasterService.success(this.resourceService.messages.smsg.m0006);
...
*/
  navigateToPage(page: number): undefined | void {
    ...
    this.route.navigate(['workspace/content/rejected', this.pageNumber], { queryParams: this.queryParams });
  }
